...
ClassDef::CompoundType convertToCompoundType(int section,uint64 specifier)
{
  ...
  else if (specifier&Entry::Annotation)
    sec=ClassDef::Annotation;
...
static void addClassToContext(const Entry *root)
{
  ...
  else // new class
  {
    ...
    if ((root->lang==SrcLangExt_CSharp || root->lang==SrcLangExt_Java || root->lang==SrcLangExt_Xtend) && (i=fullName.findRev('<'))!=-1)
    {
  ...
  if (root->section==Entry::USINGDIR_SEC)
  {
    ...
    if (!name.isEmpty())
    {
      ...
      if (root->parent() && root->parent()->section==Entry::NAMESPACE_SEC &&
          (fd==0 || (fd->getLanguage()!=SrcLangExt_Java && fd->getLanguage()!=SrcLangExt_Xtend)) // not a .java file
         )
      {
    ...
    Relationship related)
{
  ...
  if (lang==SrcLangExt_Java || lang==SrcLangExt_CSharp || lang==SrcLangExt_Xtend)
  {
...
static QCString extractClassName(const Entry *root)
{
  ...
  int i;
  if ((root->lang==SrcLangExt_CSharp || root->lang==SrcLangExt_Java || root->lang==SrcLangExt_Xtend) &&
      (i=bName.find('<'))!=-1)
  {
            ...
            if (mcd && !mcd->baseClasses().empty())
            {
              ...
              if (bmcd && mcd && bmcd!=mcd &&
                  (bmd->virtualness()!=Normal ||
                   bmd->getLanguage()==SrcLangExt_Python || bmd->getLanguage()==SrcLangExt_Java || bmd->getLanguage()==SrcLangExt_Xtend || bmd->getLanguage()==SrcLangExt_PHP ||
                   bmcd->compoundType()==ClassDef::Interface || bmcd->compoundType()==ClassDef::Annotation || bmcd->compoundType()==ClassDef::Protocol
                  ) &&
                  md->isFunction() &&
                  mcd->isLinkable() &&
                  bmcd->isLinkable() &&
...
void initDoxygen()
{
  ...
  Doxygen::parserManager->registerParser("md",           make_parser_factory<MarkdownOutlineParser>(),
                                                         make_parser_factory<FileCodeParser>());
  Doxygen::parserManager->registerParser("xtend",        make_parser_factory<XtendOutlineParser>(),
                                                         make_parser_factory<XtendCodeParser>());
