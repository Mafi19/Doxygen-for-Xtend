...
QCString ClassDefImpl::generatedFromFiles() const
{
  ...
  else if (m_impl->compType==Annotation)
  {
    result = theTranslator->trGeneratedFromFiles(Annotation, numFiles==1);
  }
  else
  {
...
{
  SrcLangExt lang = getLanguage();
  bool isIDLorJava = lang==SrcLangExt_IDL || lang==SrcLangExt_Java || lang==SrcLangExt_Xtend;
  if (isIDLorJava)
  {
    return "import";
  }
...
QCString ClassDefImpl::title() const
{
  ...
  {
    pageTitle = theTranslator->trCompoundReference(displayName(), Annotation, !m_impl->tempArgs.empty());
  }
  else
  {
